...
async function seedSampleCandidates() {
  try {
    ...
    for (const candidateData of candidates) {
      try {
        const existing = await prisma.candidate.findFirst({
          where: {
            nim: candidateData.nim,
            position: candidateData.position
          }
        });
        
        if (existing) {
          console.log(`⏭️  ${candidateData.position}: ${candidateData.name} already exists`);
          skipCount++;
          continue;
        }
    ...
    console.log(`⏭️  Skipped: ${skipCount} candidates`);
    
    console.log('\n📝 Candidates by Position:');
    console.log('━'.repeat(60));
    
    const ketuaBPH = await prisma.candidate.count({ where: { position: 'KETUA_BPH', isActive: true } });
    const senator = await prisma.candidate.count({ where: { position: 'SENATOR', isActive: true } });
    
    console.log(`Ketua BPH: ${ketuaBPH} candidates`);
    console.log(`Senator:   ${senator} candidates`);
    console.log('━'.repeat(60));
